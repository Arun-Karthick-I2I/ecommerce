...
<style>
.cartPage>form>.cartProduct>.table>tbody>tr>td {
					...
					<table class="table">
						<tbody>
							<c:forEach var="cartItem" items="${customer.cartItems}">
								<form id="${cartItem.warehouseProduct.id}">
									...
									<tr>
										...
										<td class="removeCart">
											<button type="submit" class="btn btn-danger"
												formmethod="post" formaction="/ecommerce/removeFromCart">REMOVE</button>
										</td>
										<td class="removeCheckOut"><input type="checkbox"
											name="warehouseProductId" checked
											value="${cartItem.warehouseProduct.id}" /></td>
									</tr>
								</form>
							</c:forEach>
							<tr>
								<td colspan="6">
									<button form="allProducts" class="btn btn-warning btn-lg" type="submit" 
									class="btn btn-default" formmethod="post" formaction="/ecommerce/buyProducts">
										<span><b>CONTINUE CHECKOUT &nbsp;&nbsp;<i
